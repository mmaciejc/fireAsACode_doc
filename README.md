...
Example:
If you select host only in your configuration and Global domain only, import will do diff and fetch the host configuration for Global domain. Access Policy or Device configurarion won't be fetch. 

Configuration will be saved in the provided folder.

Once the import is compleated tool will print the result details:
![fireAsACode](www_files\pic_04.jpg)

Metadata will be save in the folder, where the informations about import request would be included:

![fireAsACode](www_files\pic_05.jpg)

## Apply configuration
1. Selecting the folder with the configuration to be loaded.
You need to select the localy saved configuration folder first:
...
There is `Include delete` button which which will include and delete in `Apply` step the objects that haven't been present in JSON (locally saved configuration) but are present on FMC.
...
Once the file was uploaded, it should appear on the list of files avaliable for parsing:

![fireAsACode](www_files\pic_017.jpg)

### Parse to JSON

Once the folder was selected, the list of avaliable excel files will be listed:
